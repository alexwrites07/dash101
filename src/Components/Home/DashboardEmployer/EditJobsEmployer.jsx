  ...
  useEffect(() => {
    const fetchJobData = async () => {
      ...
      try {
        ...
        setSalaryPeriod(jobData.salary.period || "monthly");
        setExperience(jobData.experience || "");
        setCareerLevel(jobData.careerLevel || "");
        setGender(jobData.gender || "Any");
        setWorkCommitment(jobData.workCommitment || "Full-time");
        setWorkMode(jobData.workMode || "In-person");
        setKeyResponsibilities(jobData.keyResponsibilities || []);
        setSkillAndExperience(jobData.skillAndExperience || []);
        setMaxApplicants(jobData.maxApplicants || "");
        setApplicationDeadline(jobData.applicationDeadline || "");
        setAddress(jobData.location.address || "");
        setCity(jobData.location.city || "");
        setPincode(jobData.location.pincode || "");
        setCountry(jobData.location.country || "");
        setLatitude(jobData.location.coordinates[0] || "");
        setLongitude(jobData.location.coordinates[1] || "");

        // Set qualifications and categories from the job data
        setCategories(jobData.jobCategories || []);
        setQualifications(jobData.qualification || "");
        // setFilteredQualifications(jobData.qualification || []);
        // setSelectedQualifications(jobData.qualification || []);
        setQualificationInput(jobData.qualification || "");
      } catch (error) {
  ...
  const saveJobPost = async () => {
    ...
    try {
      const response = await axios.put(
        `https://server.avyudha.com/editJob/${jobId}`,
        jobData,
        {
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="flex flex-col lg:flex-row max-w-5xl">
      ...
      <div className="flex-1 mr-12">
        ...
        <div className="lg:ml-64 lg:mt-18 p-4 lg:p-28 flex flex-col w-full mr-12">
          <h1 className="text-xl font-bold mb-6">Update a Job Post</h1>
