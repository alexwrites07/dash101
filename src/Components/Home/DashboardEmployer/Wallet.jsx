...
const Wallet = () => {
  ...
  const handleRazorpayPayment = async (amount) => {
    ...
    const options = {
      key: 'rzp_test_GpP5Z2LKkBWqPx', // Replace with your Razorpay key ID
      amount: amount * 100, // Razorpay expects the amount in paise
      currency: 'INR',
      name: 'Kridhatutor',
      description: `Adding ₹${amount} to your wallet`,
      handler: function (response) {
        // Capture the response after payment is successful
            ...
            <div className="bg-white p-4 mb-6 rounded-lg shadow-md">
              ...
              {transactionHistory.map((transaction, index) => (
                <div key={index} className="mb-4">
                  ...
                  <p
                    className={`text-${transaction.amount.startsWith('-') ? 'red' : 'green'}-600`}
                  >
                    {transaction.amount}
                  </p>
                </div>
